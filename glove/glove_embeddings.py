# This code is used to run logistic regression or the random forest classifier on the embedding
# of a tweet. To get an embeddings for the whole tweet we are averaging glove word embeddings.

# The Stanford University already has precomputed glove word embeddings for Twitter:
# https://nlp.stanford.edu/projects/glove/
#
# We also have code to compute word embeddings ourselves (basically followed the CIL tutorial):
...
# We observed that the length of negative tweets is in general higher,
# if useLen is true we are using the "number of words"-information in ML models
useLen = False
# which classifier to use: True = logistic regression, False = using random forests
use_logistic_regression = True


# Method taken from the CIL tutorial:
# https://github.com/dalab/lecture_cil_public/tree/master/exercises/ex6
...
def read_input_data(filename, matrix_list, embeddings, vocab, is_test_data, labels=None):
    ...
    with open(filename, 'r+', encoding="utf-8") as f:
        for line in f:
            ...
            # At the moment: if we do not have an embedding for any of the words in the tweet
            # -> do not use them for training (are not that many)
            # Compute the average
            if (len(tokens) > 0):
...
def main():
    ...
    if compute_embedding and not use_stanford:
        ...
        # Compute a mapping from the word to the index
        # (to be able to get the right embedding for a given word)

    if use_stanford:
        print("Loading Stanford 200D Vocab...")
        stanford_label = genfromtxt(
            'glove.twitter.27B.200d.txt', delimiter=' ', encoding="utf8", usecols=(0), dtype=None)
    ...
    read_input_data('train_sent.txt', matrix_list, glove_words_embeddings, vocab, False,
                    train_labels)  # label deactivated
    ...
    read_input_data('eval_sent.txt', matrix_list, glove_words_embeddings, vocab, False,
                    eval_labels)  # label deactivated
    ...
    output = np.concatenate(
        (np.expand_dims(np.arange(1,
                                  len(predicted_results) + 1), 0),
         np.expand_dims(predicted_results, 0)),
        axis=0).T
    # Save to file
    np.savetxt(
        output_file_name, output, delimiter=',', fmt="%d,%d", header="Id,Prediction", comments='')
