...
class SentimentTwitter(text_problems.Text2ClassProblem):
  ...
  def doc_generator(self, imdb_dir, dataset, include_label=False):
      print("generating eval and train")
      files = [(os.path.join(imdb_dir, "{}_{}_shufsplit.txt".format(dataset, "pos")), True), 
               (os.path.join(imdb_dir, "{}_{}_shufsplit.txt".format(dataset, "neg")), False)]
  ...
  def generate_samples(self, data_dir, tmp_dir, dataset_split):
    ...
    if dataset_split == problem.DatasetSplit.TRAIN:
      ...
      dataset = "eval"
      include_label=True
    for doc, label in self.doc_generator(imdb_dir, dataset, include_label=include_label):
      yield {
          "inputs": doc,
          "label": int(label)
      }
